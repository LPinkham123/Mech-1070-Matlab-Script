...
for i = 1:size(cube_coords, 1)
    ...
    if(i == 5)
        ...
        pause(6); 
        fprintf(s, 'G1 z0.5 f10'); % Drops to last cube for pick up
        pause(); % Pauses until Keypress -- Servo Lift
        fprintf(s, 'G1 z2.5 f10'); % Lifts to move cube
        pause(6);
        fprintf(s, ['G1 x', num2str(dest_coords(i, 2)), ' y', num2str(dest_coords(i, 1)), ' f10']); % Goes to last cube destination
        pause(6);
        fprintf(s, 'G1 z1 f10'); % Down to drop stacked cube
